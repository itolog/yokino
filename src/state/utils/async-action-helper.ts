...
export function asyncActionHandlerFactory<PayloadType, ReturnedType, ErrorType>(type: string, concurrencyType?: ConcurrencyType) {
    ...
    action$.pipe(
      ...
      concurrencyOperator((action: PayloadMetaAction<PayloadType>) =>
        requestFn(action.payload).pipe(
          ...
          catchError(pipe((error) => Actions.failure(error, action.meta), of)),
          takeUntil(action$.pipe(filter((cancelAction: Action) => cancelAction.type === ActionTypes.ACTION_CANCELED))),
        ),
      ),
    );

  const reducer = (state: AsyncActionState = asyncActionInitialState, action: PayloadAction<ReturnedType | ErrorType>): AsyncActionState => {
    switch (action.type) {
